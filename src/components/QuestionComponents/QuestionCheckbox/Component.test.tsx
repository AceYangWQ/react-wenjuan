...
test('默认属性', () => {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    expect(checkbox.getAttribute('checked')).toBeNull() // 每一个 checkbox 都默认 false
  }
})

test('传入属性', () => {
  const options = [
    { value: 'v1', label: 'l1', checked: false },
    { value: 'v2', label: 'l2', checked: true },
    { value: 'v3', label: 'l3', checked: true },
  ]
  render(<Component title="hello" optionList={options} />)
